...
class FDSA(SequentialRecommender):
    ...
    def calculate_eta(self, logits, pos_items):
        ...
        # find the probability of the target item
        cal_scores = 1 - prob[torch.arange(prob.size(0)), pos_items]
        # calculate the cumulative sum of the probability
        target_probability = torch.quantile(cal_scores, self.alpha, interpolation='higher')
        return target_probability

    def _calculate_comparsion_result(self, cor_cal_logits, cor_cal_pos_items, cor_test_logits):
        # get the treshold from the training (fine tuning) data
        threshold = self.calculate_eta(cor_cal_logits, cor_cal_pos_items)
        # convert logits to probability
        prob = nn.functional.softmax(cor_test_logits, dim=1)
    ...
    def calculate_set_size_loss(self, comparison_result, prob):
        ...
        # prediction_sets = prob >= (1-threshold)
        # C_score = torch.sum(prediction_sets,dim=1)
