...
class UniSRec(SASRec):
    ...
    def calculate_loss(self, interaction):
        ...
        item_seq = interaction[self.ITEM_SEQ]
        # since we merge the train and valid sequences, we need to mask the valid sequences
        item_seq_len = interaction[self.ITEM_SEQ_LEN] - 1

        # Replace values at the specified indices with 0
        train_item_seq = item_seq.scatter(1, item_seq_len.unsqueeze(1), 0)

        # Extract the values at item_seq_len - 1 index for each row
        pos_items = item_seq.gather(1, item_seq_len.unsqueeze(1)).squeeze(1)
        
        train_item_emb_list = self.moe_adaptor(self.plm_embedding(train_item_seq))
        train_seq_output = self.forward(train_item_seq, train_item_emb_list, item_seq_len)
